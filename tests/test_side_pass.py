...
from test_avoid import (HZ, STATIC_TICKS, Ap, Box, GeomPlanner,     # noqa: E402
                        LgOne, Planner, _geom_rig, make, try_overtake)
...
def test_params_present():
    ...
    assert STANDOFF == 25.0 and OT['standoff_stop_s'] == 1.0
...
def test_standoff_kill_switch_uses_static_corridor():
    ...
    kr, p, ap = _two_object_rig(cfg, stop_ticks + 2)
    try_overtake(kr, ap, p, ego_speed=0.0)
    assert kr.wait_target_d == pytest.approx(40.0)                 # 이전 동작: 정적 축 corridor[0]
    assert kr.standoff_id in (None, 1)                              # legacy 는 None, queue_only 는 id
