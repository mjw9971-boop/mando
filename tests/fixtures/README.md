...
## `pathshape03_finish_straight_waypoints.csv`

`pathshape03_waypoints.csv` 에서 **종료점(seq 10)만** 옮긴 사본이다.

원본의 종료점은 좌회전 **연결로** `(1502,0,-1)`(junction 4, 폭 2.70 m) 안
s=865.4 에 앉는다. 주최측이 종료 지점에 놓는 라바콘이 그 좁은 회랑에 들어와
실주행 2차(20260908_222954/실경로_02)에서 종료선 3.7 m 앞 483틱 정지(미완주)를
냈다. 대회 형식상 **종료점은 직선 위**이므로 연결로 종료는 비현실적인
전제였다 — 그 전제를 뺀 대조군이 이 파일이다.

옮긴 곳: 같은 연결로를 빠져나온 직후의 직선 `(1492,0,-1)`(junction −1) s=869.1.
`(415.672, -349.703)` 는 그 차로 중심선 위의 점이라 경유점 이탈 0.00 m 다.
seq 9 가 s=841.2(연결로 진입)라 종료점은 그보다 **뒤**여야 한다 — 더 앞의 넓은
직선 `(100,0,2)` 로 옮기면 경로가 되돌아 도느라 896.7 → 1096.5 m 가 된다.

빌드 결과는 원본과 같다 (896.7 m / 차로 31 / rc=0). 종료 s 만 865.4 → 869.1,
꼬리 31.3 → 27.5 m (둘 다 `finish_tail_m` 12 이상).

쓰는 곳: `tests/test_finish_gate_pdm.py` — 연결로 종료 / 직선 종료 대조.
