...
여기서 지키는 계약:
  ...
  3. 킬 스위치 route.valid_entry_lanes_enable=false 면 필드 자체가 없다.
...
def test_kill_switch_removes_field(lg, monkeypatch):
    """false 면 필드가 아예 없고, 나머지 필드는 그대로다."""
...
def test_covers_every_non_pair_segment(lg, csv):
...
@pytest.mark.parametrize('csv', CSVS)
def test_last_segment_is_finish_with_empty_set(lg, csv):
...
@pytest.mark.parametrize('csv', CSVS)
def test_entry_pool_excludes_junction_lanes(lg, csv):
...
def test_turns_narrow_the_set(lg):
...
def test_official_route_sets(lg):
...
def test_empty_pair_set_means_banned_connector(lg):
...
def test_reuses_turn_connect(lg, monkeypatch):
