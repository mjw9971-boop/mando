"""
obstacle_chain 이 **한 가용구간 안에** 통째로 놓이는지 (2026-09-02 회귀).

결함: pick_s 는 구간들을 이어붙인 길이축에서 비율 지점을 고를 뿐인데, 배치는
s0 + i*spacing 으로 raw route_s 를 따라갔다. 그래서 체인 꼬리가 구간 밖 —
교차로 안으로 걸어 나갔다. 실측(표본 144): 개수 6 → 30% / 4 → 14~18% /
3 → 5% 가 교차로 내부에 장애물을 놓았다. 교차로 안 장애물은 회피 연습이
아니라 채점 왜곡이다.

수정: _place_chain 이 체인 전체가 들어가는 구간만 후보로 삼고, 요청 위치를
품은 구간 → 가까운 구간 순으로 옮겨 보다가, 전부 실패해야 개수를 줄인다.
"""
import pathlib
import random
import sys
...
class FakeCtx:
    """_place_chain 이 쓰는 표면만 — 가용구간과 점유 구간."""
...
def test_chain_never_straddles_a_span_boundary():
    ...
    s0 = gs._place_chain(FakeCtx(spans), 4, SPACING, 0.9)
...
def test_all_fracs_and_counts_stay_inside(frac, n):
    ...
    s0 = gs._place_chain(FakeCtx(spans), n, SPACING, frac)
...
def test_falls_back_to_another_span_before_shrinking():
    ...
    s0 = gs._place_chain(ctx, 4, SPACING, 0.1)          # 요청은 첫 구간 쪽
...
def test_returns_none_when_no_span_can_hold_the_chain():
    ...
    assert gs._place_chain(FakeCtx([(60.0, 100.0), (200.0, 240.0)]), 6, SPACING, 0.5) is None
...
def test_shrinks_only_after_every_span_failed():
    ...
    assert gs._place_chain(FakeCtx(spans), 6, SPACING, 0.5) is None
    s0 = gs._place_chain(FakeCtx(spans), 4, SPACING, 0.5)
...
def test_claim_width_unchanged():
    ...
    s0 = gs._place_chain(ctx, 4, SPACING, 0.5)
