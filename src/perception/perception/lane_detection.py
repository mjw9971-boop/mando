...
class _Stabilizer:
  ...
  def update(self, center_error, is_valid, dt_s):
    ...
    self.ema = (
        center_error if self.ema is None else a * center_error + (1 - a) * self.ema
    )
    return self.ema, 'OK'
...
class LaneDetection(Node):
  ...
  def __init__(self):
    ...
    self.get_logger().info('🛣️ [Lane Detection] 차선 인식 노드 시작')
    ...
    self.pub_lane_offset = self.create_publisher(
        Float32, '/perception/lane_offset', 10
    )
  ...
  def process_lane_detection(self, cv_image, dt_s):
    ...
    yellow_mask = cv2.inRange(
        img_lab, np.array([0, 130, 140]), np.array([255, 170, 255])
    )
    white_mask = cv2.inRange(
        img_hls, np.array([0, 200, 0]), np.array([255, 255, 255])
    )
    ...
    bev_img = cv2.warpPerspective(color_mask, bev_matrix, (w, h))
    ...
    ema_pixel_offset, _ = self.stabilizer.update(pixel_offset, is_valid, dt_s)
