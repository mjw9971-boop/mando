...
DEFAULTS: dict[str, Any] = {
    ...
    'vehicle': {'wheelbase': 2.944, 'max_steer': 0.48, 'length': 4.848,
                ...
                # 경로 계획의 연결로 통행 금지 임계: R_min < (축거/tan(최대조향)) x 이 값.
                # 1 미만인 이유: 차로 폭 여유(t_off ±1 m)를 쓰면 기하 최소반경(5.65 m)보다
                # 작은 호도 통과된다. 실증 — R 4.52 m 연결로(회전1)는 여러 런에서 통과
                # (포화 1.2 s, t_off 최대 0.85 m), R 2.55 m(1576)는 이탈·탈선.
                # 0.7 → 임계 3.96 m: 전자 허용, 후자 금지. (build_route 소비)
                'min_turn_margin': 0.7,
    ...
    'lead': {'time_headway_s': 2.0, 'min_gap_m': 5.0,
             ...
             # 추월 대상 뒤 정지 간격. **횡이동이 그 차 뒤에서 끝나야 하는 기하 조건**:
             # smoothstep 블렌드가 w≈1 이 되는 데 전이거리 L 만큼 전진이 필요하고,
             # L 은 최소 회전 기하(축거 2.944, 풀락 0.48 → S 커브 약 10 m) 아래로
             # 줄일 수 없다. 8 m 로 서면 옆으로 다 넘어가기 전에 그 차 위치에
             # 도달한다 (2026-08-24 폐루프: 최소 접근 1.91 m 충돌). 12 m 면
             # 뒷범퍼 도달 시점에 횡이동이 끝난다.
             'blocked_gap_m': 12.0,
    ...
    'lane_change': {'back_m': 30.0, 'front_m': 50.0, 'min_window_m': 20.0,
                    ...
                    'v_min_mps': 2.0,
                    # blocked 추월(정지 출발) 전용: IDM 정지 목표를 풀고 이 속도로
                    # 기어 나간다. 지시등을 채우는 동안 IDM 이 blocked_gap 앞 정지를
                    # 만들면 v_min 게이트와 교착이 된다 (2026-08-24 13:10 런).
                    'creep_kph': 8.0,
                    # 전이 중 v≈0 인 채 이 시간 이상 진행이 없으면 중단·복귀 (교착 방지)
                    'stall_abort_s': 3.0},
    'cross': {'margin_s': 2.0},
