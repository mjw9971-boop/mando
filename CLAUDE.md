...
- **경로를 바꾸는 변경은 `build_route.report()` 의 `rc` 까지 본다.** 빌드가
  예외 없이 끝나는 것과 경로가 쓸 만한 것은 다르다 — `rc≠0` 이면 배치가 그
  경로를 버린다. 2026-09-09 회전 차로 제약을 켤 때 빌드 예외만 세고 rc 를 안 봐서
  **신규 rc≠0 4건**(`lane_change_right window 13.5 m < MIN_LC_WINDOW_M 20`)을
  놓친 채 커밋했다. 전수 판정은 항상 두 축이다: **빌드 실패 수 + rc≠0 수**.
- **커밋 전에 `git status` 로 들어갈 파일을 눈으로 확인한다.** `git add -A <디렉터리>`
  는 그 순간의 모든 수정을 쓸어담는다 — 2026-09-09 에 검증용으로 잠시 켜 둔
  `avoid_map.lane_map_avoid_enable: true` 가 그렇게 딸려 들어가, "켜지 말 것" 으로
  판정한 기능이 켜진 채 커밋됐다 (다음 회귀 비교의 기준선까지 오염됐다).
  **실험용 스위치를 켠 상태에서는 특히 `git add -A` 를 쓰지 말 것.**
...
- `pytest` 는 이제 이 환경에 **설치되어 있다** (7.4.4). 예전의 "고정 실패
  ...
  **현재 기준선 (2026-09-07 실측, `python3 -m pytest -q`, ~7.6 분)**:
  `1320 passed / 2 skipped / **0 failed**`. 전부 통과한다.
...
### `_raw_accel` 의 `err/dt` 는 **PDM IDM 전용**이다 — 상한형에 쓰면 안 된다

`VtdLongitudinalController._raw_accel` 의 감속측

```
err < 0  →  max(err / dt, a_dec_max)      # dt = 0.05
```

는 **PDM IDM 의 target 이 "1틱 전방 속도"** 라서 성립하는 식이다 —
`(target − v)/dt` 가 곧 IDM 이 의도한 감속이라 1/dt = 20배 증폭이 **의도된**
것이다. 정지선·보행자·standoff 처럼 목표점까지 프로파일을 그리는 축도 여기
속한다.

**상한형 목표는 그 전제가 없다.** 곡률(`speed.curvature_cap`) · 차선변경
(`lc_speed_cap`) · 붉은구간 진입 전 감속 · **순수 제한속도**(PDM 의 중재 전
목표를 아무도 안 줄인 틱)는 "이 속도를 넘지 마라" 일 뿐이다. 같은 축에 태우면:

- 목표가 실속도보다 **0.20 m/s** 낮기만 해도 `a_dec_max`(−4.0) 포화
- 포화를 jerk 램프(+0.1/틱)로 푸는 데 **1.75 s**
- 그 사이 상한 **아래로** 언더슛 → 다음 창에서 재포화 = **되먹임**

2026-09-09 실측(20260908_222954/실경로_01): 이 꼴이 한 주행에 **11구간**,
min accel −3.9, rs 363.8~366.0 은 목표 5.56 m/s 인데 **완전 정지**했다.

**상한형은 P 제어 + `control.a_cap_dec_max`(−1.5) 클램프다.** 수정 후 같은
11구간이 **−3.90 → −0.96**, off 는 로그와 bit 동일(3610/3610 · 2717/2717),
전 주행 min accel 은 −4.0 그대로다(정지 축은 안 건드렸다는 확인).

새 감속 후보를 만들 때 **먼저 물을 것**: 이건 "1틱 뒤에 이 속도가 되어라"
인가, "이 속도를 넘지 마라" 인가. 후자면 `cap_target` 축이다
(`kr_rules` 의 `cap_binds` — 상한형이 최종 목표를 정한 틱에만 선다).
